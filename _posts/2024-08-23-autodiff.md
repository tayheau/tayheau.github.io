...
![](/assets/img/autodiff/error_fd_diff.png)
...
| \\(\dot{w_0}\\)        | \\(\dot{a} \cdot cos(a) = cos(a)\\)                   |\\(-0.416\\)        |
| \\(\dot{w_1}\\) | \\(\dot{b} \cdot w_0 + b \cdot \dot{w_0} = b \cdot \cos(a)\\) | \\(-2.081\\) |
| \\(\dot{w_2}\\)        | \\(2 \cdot \dot{b}\\)                                 | \\(0\\)    |
| \\(\dot{r}\\)                   |\\(\dot{w_1} + \dot{w_2} = b \cdot cos(a)\\)    |  \\(-2.081\\)      |

As you can see, this can be generalized to compute one column of the Jacobian matrix of a function \\(f:\mathbb{R}^n \to \mathbb{R}^m\\) at a given point \\(a\\) by setting \\(\dot{y_j} = \dfrac{\partial y_j}{\partial x_i}\Bigg\vert_{x = a}\text{, }j=1, \ldots, m\\)
...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| \\(\bar{r}\\)         | \\(\bar{r}\\)                                                                                                                  | \\(1\\)                                   |
| \\(\bar{w_1}\\)       | \\(\bar{w_1}\\) += \\(\dfrac{\partial r}{\partial r}  \cdot \dfrac{\partial r}{\partial w_1} = \dfrac{\partial r}{\partial r} = \bar{r}\\) | \\(1\\)                                   |
| \\(\bar{w_2}\\)       | \\(\bar{w_2}\\) += \\(\bar{r}\\)                                                                                                     | \\(1\\)                                   |
| \\(\bar{b}\\)         | \\(\bar{b}\\) += \\(\bar{w_2} \cdot 2b\\)               = \\(2b\\)                                                                                        |\\(10\\)                                       |
| \\(\bar{w_0}\\)       | \\(\bar{w_0}\\) += \\(\bar{w1} \cdot b\\)                = \\(b\\)                                                                                       |\\(5\\)                                       |
| \\(\bar{b}\\)         | \\(\bar{b}\\) += \\(\bar{w1} \cdot w_0\\)    = \\(10 + sin(a)\\)                                                                                      |\\(10.909\\)                                       |
